...
def is_project_empty(name, remote):
    ...
    try:
        ...
        if len(commits) > 0:
            if remote != "aosp":
                branche = gl_project.branches.get("hxos16.0.1")
                if branche is None:return True
...
if __name__ == '__main__':
    parse_and_check(config.LINEAGE)
    parse_and_check(config.MANIFEST_PATH)
    parse_and_check(config.roomservice)
